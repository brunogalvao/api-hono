...
app.get("/api/groups/:id/members", async (c) => {
  ...
  // Verifica se o usuário pertence ao grupo (RLS já garante, mas retornamos 403 explícito)
  ...
  if (!membership) return c.json({ error: "Grupo não encontrado ou acesso negado." }, 403);
