...
const analiseLocalInteligente = (
  ...
  if (rendimentoDisponivel < 0) {
    ...
    return {
      ...
      analiseDetalhada: {
        ...
        nivelRisco:
          fatorRisco > 3 ? "EXTREMO" : fatorRisco > 2 ? "ALTO" : "MÉDIO",
      },

      estrategiaInvestimento: {
        curtoPrazo: "🚨 EMERGÊNCIA: Cortar gastos em 80% imediatamente",
        medioPrazo: `💰 URGENTE: Aumentar renda em pelo menos ${formatToBRL(deficitMensal)}`,
        longoPrazo: "📊 CRÍTICO: Renegociar todas as dívidas pendentes",
      },
      estrategiaRecuperacao: {
        prioridade1: "🚨 EMERGÊNCIA: Cortar gastos em 80% imediatamente",
        prioridade2: `💰 URGENTE: Aumentar renda em pelo menos ${formatToBRL(deficitMensal)}`,
        prioridade3: "📊 CRÍTICO: Renegociar todas as dívidas pendentes",
  ...
  if (
    ...
    return {
      ...
      estrategiaInvestimento: {
        curtoPrazo: "💰 Criar reserva de emergência de 6 meses",
        medioPrazo: "📈 Aumentar renda em 20-30% para investimentos",
        longoPrazo: "📊 Otimizar gastos para liberar 15% da renda",
      },
  ...
  if (rendimentoDisponivel > rendimentoMes * 0.1) {
    const percentualDisponivel = (rendimentoDisponivel / rendimentoMes) * 100;
    ...
    return {
      ...
      estrategiaInvestimento: {
        curtoPrazo: `Investir ${formatToBRL(reservaEmergencia)} em reserva de emergência`,
        medioPrazo: `Aplicar ${formatToBRL(investimentoSeguro * 0.4)} em CDB e fundos conservadores`,
        longoPrazo: `Diversificar ${formatToBRL(investimentoSeguro * 0.6)} em fundos e dólar para crescimento`,
      ...
      melhoriaEconomica: {
        potencialMaximoInvestimento: formatToBRL(rendimentoDisponivel * 0.8),
  ...
  return {
    ...
    rendimentoDisponivel: rendimentoDisponivel,
    estrategiaInvestimento: {
      curtoPrazo: "Análise em processamento...",
      medioPrazo: "Análise em processamento...",
      longoPrazo: "Análise em processamento...",
    },
...
const tentarGemini = async (prompt: string, tentativas: number = 3) => {
  ...
  for (let tentativa = 1; tentativa <= tentativas; tentativa++) {
    try {
      ...
      const modelosTeste = ["gemini-1.5-flash", "gemini-1.5-pro", "gemini-pro"];
      ...
      console.log(`❌ Erro no Gemini (tentativa ${tentativa}):`, error.message);
      ...
      if (
        ...
        console.log("🔄 Servidor sobrecarregado - tentando novamente...");
        if (tentativa < tentativas) {
          const delay = Math.min(1000 * Math.pow(2, tentativa - 1), 10000); // Exponential backoff
        ...
        if (tentativa < tentativas) {
          console.log("🔄 Tentando novamente com parsing diferente...");
          continue;
        }
      } else {
...
app.post("/api/ia/analise-investimento", async (c) => {
  try {
    ...
    if (incomesError) {
      ...
      return c.json({ error: "Erro ao buscar dados de rendimentos" }, 500);
    }
    ...
    const rendimentosDoMes =
      incomes?.filter((income) => {
        if (income.mes && income.ano) {
          return income.mes === targetMonth && income.ano === targetYear;
        }
    ...
    if (!finalCotacaoDolar) {
      try {
        ...
        console.warn("Erro ao buscar cotação do dólar, usando valor padrão");
      ...
      return c.json(
        {
          ...
          suggestion: "Use POST /api/incomes para adicionar rendimentos",
        },
        400,
      );
    }
