...
| `POST` | `/api/workspaces/:id/invites/:inviteId/resend` | Required | Rotate the token and resend a pending invitation. Body: `locale`. |
| `DELETE` | `/api/workspaces/:id/invites/:inviteId` | Required | Cancel a pending invitation. |
...
The API stores only a SHA-256 token hash in the database. Token rotation, rate limiting, delivery recording, cancellation and acceptance are implemented by the migration `20260817120453_secure_workspace_invites.sql` and its transactional RPCs.
